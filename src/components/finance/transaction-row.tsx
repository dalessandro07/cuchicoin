...
function dateTimeLabel(seconds: number): string {
...
export function TransactionRow({
	...
	const when = dateTimeLabel(transaction.createdAt);
	...
	return (
		...
		>
			<View
				style={[styles.iconCircle, { backgroundColor: `${circleColor}22` }]}
			>
			...
			<View style={styles.body}>
				...
				<Text
					style={[styles.when, { color: theme.textSecondary }]}
					numberOfLines={1}
				>
					{when}
				</Text>
			...
			<Text style={[styles.amount, { color: amountColor }]}>
				{hidden
					? "••••••"
					: formatSignedSoles(transaction.amount, transaction.type)}
			</Text>
		</Pressable>
	);
}
