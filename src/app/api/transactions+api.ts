...
export const GET = handle(async (request) => {
  ...
  const limit = limitParam ? Math.min(Math.max(Number(limitParam), 1), 200) : undefined;

  const items = await listTransactions({ homeId, month, limit });
...
export const POST = handle(async (request) => {
  ...
  if (body.categoryId) {
    const [category] = await db
      .select()
      .from(categories)
      .where(and(eq(categories.id, body.categoryId), eq(categories.homeId, homeId)))
      .limit(1);
