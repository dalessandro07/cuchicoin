...
import { serializeCategory, serializeHome, serializeMember } from '@/lib/api-serialize';
...
export const GET = handle(async (request, { id }) => {
  ...
  const [homeMembers, homeCategories, recentTransactions, balance] = await Promise.all([
    db.select().from(members).where(eq(members.homeId, id)).orderBy(asc(members.joinedAt)),
    db
      .select()
      .from(categories)
...
export const DELETE = handle(async (request, { id }) => {
  ...
  const homeMembers = await db.select().from(members).where(eq(members.homeId, id));
  ...
  if (membership.role === 'admin' && otherAdmins.length === 0) {
    if (others.length > 0) {
      throw new ApiError(409, 'Promueve a otro miembro a administrador antes de salir');
    }
  ...
  await db.delete(members).where(and(eq(members.homeId, id), eq(members.userId, user.id)));
