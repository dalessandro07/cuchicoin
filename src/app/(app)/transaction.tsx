...
function TransactionForm({ initial }: { initial: FormInitial }) {
  ...
  const { categories, createTransaction, updateTransaction, deleteTransaction } = useHome();
  ...
  const onSave = async () => {
    ...
    try {
      if (isEdit && initial.id) {
        await updateTransaction(initial.id, { type, amount: cents, categoryId, description });
      } else {
        await createTransaction({ type, amount: cents, categoryId, description });
      }
      router.back();
    } catch (err) {
      Alert.alert('Error', err instanceof Error ? err.message : 'No se pudo guardar el movimiento');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        <View style={[styles.amountCard, { backgroundColor: theme.backgroundElement, borderColor: theme.border }]}>
          <TypeToggle value={type} onChange={changeType} />
          <AmountInput value={amountText} onChangeText={setAmountText} tint={tint} autoFocus={!isEdit} />
        ...
        <CategoryPicker
          ...
          onManage={() => router.push('/(app)/categories')}
        />
...
function EditLoader({ id }: { id: string }) {
  const theme = useTheme();
  const { data, loading, error } = useAsyncData(() => financeApi.getTransaction(id), `txn:${id}`);

  if (loading) return <ActivityIndicator style={styles.loader} color={theme.primary} />;
  if (error || !data) {
    return <EmptyState icon="cloud-offline-outline" title="No se pudo cargar" message={error ?? undefined} />;
  }
...
export default function TransactionModal() {
  ...
  const paramType: CategoryType = params.type === 'income' ? 'income' : 'expense';

  const title = id ? 'Editar movimiento' : paramType === 'income' ? 'Nuevo ingreso' : 'Nuevo gasto';
    ...
    <ThemedView style={styles.container}>
      <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
        ...
        {id ? (
          ...
          <TransactionForm
            initial={{ type: paramType, amountText: '', categoryId: null, description: '' }}
          />
        )}
      </SafeAreaView>
    </ThemedView>
