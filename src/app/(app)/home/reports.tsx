...
function MonthReport({ homeId, hidden }: { homeId: string; hidden: boolean }) {
	...
	const { data: months } = useAsyncData(
	...
	const memberEntries = Array.from(memberMap.values()).sort(
			...
			{loading || !summary ? (
				...
				<View style={styles.content}>
					<View style={[styles.card, { backgroundColor: theme.primary }]}>
						<Text style={[styles.cardLabel, { color: theme.primaryContrast }]}>
							Balance del mes
						</Text>
										...
										{memberEntries.map((m, idx) => (
											...
											>
												...
												<View style={styles.memberAmounts}>
													<Text
														style={[
															styles.memberExpense,
															{ color: theme.text },
														]}
													>
														{hidden
															? "••••"
															: `- ${formatSoles(m.expense, { withSymbol: false })}`}
													</Text>
													<Text
														style={[
															styles.memberIncome,
													...
													>
														{hidden
															? "••••"
															: `+ ${formatSoles(m.income, { withSymbol: false })}`}
													</Text>
												</View>
											</View>
										))}
...
export default function ReportsScreen() {
	const { currentHome, dataVersion, refresh } = useHome();
	...
	return (
		<ThemedView style={styles.container}>
			<SafeAreaView style={styles.safe} edges={["top"]}>
				...
				>
					<MonthReport
						key={`${currentHome.id}:${dataVersion}`}
