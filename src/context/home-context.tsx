...
export type HomeContextValue = {
  ...
  updateTransaction: (id: string, input: Partial<CreateTransactionArgs>) => Promise<void>;
  deleteTransaction: (id: string) => Promise<void>;
  createCategory: (input: CreateCategoryArgs) => Promise<Category>;
  updateCategory: (id: string, input: Partial<Omit<CreateCategoryArgs, 'type'>>) => Promise<Category>;
  deleteCategory: (id: string) => Promise<void>;
};

const EMPTY_BALANCE: Balance = { incomeCents: 0, expenseCents: 0, balanceCents: 0 };
...
export function HomeProvider({ children }: { children: ReactNode }) {
  ...
  const key = authStatus === 'authenticated' && sessionUserId ? sessionUserId : authStatus;
  useKeyedEffect(key, () => {
  ...
  const createHome = useCallback<HomeContextValue['createHome']>(
    async (input) => {
      const { home } = await financeApi.createHome(input.name.trim());
  ...
  const joinHome = useCallback<HomeContextValue['joinHome']>(async (input) => {
    const { home } = await financeApi.joinHome(input.inviteCode.trim().toUpperCase());
    const loaded = await financeApi.getHomeDetail(home.id);
    setDetail(loaded);
    setStatus('in-home');
    setDataVersion((v) => v + 1);
