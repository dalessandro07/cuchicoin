...
function sessionToAppSession(
	...
	return {
		user: {
			id: session.user.id,
			firstName:
				(user.firstName as string) ?? session.user.name?.split(" ")[0] ?? "",
			lastName:
...
export function AuthProvider({ children }: { children: ReactNode }) {
	...
	const appSession: AppSession | null = session
		? sessionToAppSession(session)
		: null;
	...
	const signUp = useCallback(async (input: RegisterForm) => {
		...
		// ECUS01: after register, end the auto-session so the user lands on login.
